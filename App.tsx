...
const App: React.FC = () => {
  // ⚡ BOLT OPTIMIZATION: Switched from controlled state (useState) to uncontrolled ref (useRef).
  // Why: Prevents the entire App component (and all its complex children) from re-rendering on every single keystroke.
  // Impact: Measurably reduces React render cycles from O(n) (where n is characters typed) to O(1) (only on submit/focus).
  // Memory footprint is reduced and main thread blocking is eliminated during typing.
      ...
      if (typeof chrome !== 'undefined' && chrome.storage) {
        chrome.storage.local.get(['lastSelectedText', 'chalamandra_history'], (data) => {
          if (data.lastSelectedText) {
            if (inputRef.current) {
              inputRef.current.value = data.lastSelectedText;
            }
  ...
  const saveToHistory = (newResult: DialecticalState, currentInput: string) => {
          ...
          <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 scrollbar-hide">
            {history.length === 0 ? (
              ...
              <div
                ...
                onClick={() => {
                  if (inputRef.current) {
                    inputRef.current.value = item.input;
                  }
        ...
        <>
          <main className="space-y-5">
            <div className="relative group">
              {/* ⚡ BOLT: Uncontrolled textarea. Value is only read on submit via inputRef.current.value */}
              <textarea
                ref={inputRef}
