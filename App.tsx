...
const App: React.FC = () => {
  // ⚡ Bolt Optimization: Using useRef for the main input instead of useState
  // prevents the entire App component (and its complex children) from re-rendering
  // on every single keystroke. We only need the value when processing/saving.
  ...
  const saveToHistory = (newResult: DialecticalState, originalInput: string) => {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      saveToHistory(dialecticResult, inputValue);
  ...
  const handleDisruption = async () => {
    ...
    try {
      ...
      saveToHistory(disruptionResult, inputValue);
      setStatus({ step: 'complete' });
    } catch (e) {
